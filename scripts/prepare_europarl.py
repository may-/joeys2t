...
def process(data_root, src_lang, trg_lang):
    ...
    for split in SPLITS:
        ...
        dataset_dict[split] = load_dataset("europarl_st",
                                           split=split,
    ...
    for split in SPLITS:
        dataset_dict[split] = dataset_dict[split].map(
            lambda row: {
                ...
                zip_manifest[row["id"]],
                f"{src_lang}_utt":
                row["sentence"].lower() if LOWERCASE[src_lang] else row["sentence"],
                f"{trg_lang}_utt":
...
def main():
    ...
    parser.add_argument("--src_lang", default="de", required=True, type=str)
    parser.add_argument("--trg_lang", default="en", required=True, type=str)
